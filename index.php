...
$usuario_es_admin = ($usuario_autenticado && isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin');
...
if ($usuario_es_admin) {
    $sql_top = "SELECT
                    tv.id_ubicacion,
                    u.nombre,
                    COUNT(tv.id_ubicacion) AS visitas
                FROM
                    top_visitado AS tv
                JOIN
                    ubicaciones AS u ON tv.id_ubicacion = u.id
                GROUP BY
                    tv.id_ubicacion, u.nombre
                ORDER BY
                    visitas DESC
                LIMIT 3";
...
<body>
    ...
    <div class="search-container">
        ...
        <?php if ($usuario_es_admin && !empty($top_visitados)): ?>
            <section class="top-visited-section">
                <h3>Top 3 Mas Visitados</h3>
                <ol class="top-visited-list">
                    <?php foreach ($top_visitados as $item): ?>
                        <li>
                            <span class="top-name"><?php echo htmlspecialchars($item['nombre']); ?></span>
                            <span class="top-count"><?php echo $item['visitas']; ?> visitas</span>
                        </li>
                    <?php endforeach; ?>
                </ol>
            </section>

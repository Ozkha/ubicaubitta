...
const allCategorySections = document.querySelectorAll(".category-section");
 ...
 function aplicarFiltro(termino) {
        ...
        allLocationItems.forEach((item) => {
            ...
            const elementoNombre = item.querySelector(".location-details p") || item.querySelector(".history-name");

            if (elementoNombre) {
                nombreLugar = elementoNombre.textContent.toLowerCase();
            }

            if (terminoBusqueda === "" || nombreLugar.includes(terminoBusqueda)) {
        ...
        allCategorySections.forEach((section) => {
            if (section.classList.contains('history-section')) return; 

            const itemsVisibles = section.querySelectorAll('.location-item[style*="display: flex"]');
